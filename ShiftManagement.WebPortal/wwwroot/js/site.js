...
/*  Changes start 07-11-2023 
    Done by Binoy
*/
let Dttbl_moduleOneData = $('#Dttbl_vieweditrequests').DataTable({
    ...
    autoWidth: false,
    ajax: {
        url: '/ViewEditRequests?handler=ViewEditRequestsData',
        dataSrc: ''
    },
    columns: [
        { data: 'requestId', visible: false },
        { data: 'requestNumber', width: '4%' },
        { data: 'formattedReceivedOnDate', width: '4%' },
        { data: 'typeOfReqId', width: '6%' },
        { data: 'requestFrom', width: '6%' },
        { data: 'requestTo', width: '6%' },
        { data: 'subProgrammeId', width: '6%' },
        { data: 'staffResponsibleId', width: '4%' },
        {
            data: 'responded', width: '4%', className: "align-items-center" ,
            "render": function (data, type, row) {
                var dval = data;
                if (dval == "Yes") {
                    return "<div class='d-flex align-items-center text-success'><i class='bx bx-radio-circle-marked bx-rotate-90 align-middle font-18 me-1'></i><span>" + dval + "</span></div>"
                }
                else {
                    return "<div class='d-flex align-items-center text-danger'><i class='bx bx-radio-circle-marked bx-burst bx-rotate-90 align-middle font-18 me-1'></i><span>" + dval + "</span></div>"
                }
            }
        },
        { data: 'respondedThroughId', width: '4%' },
        { data: 'respondedYear', width: '4%', className: "text-center" },
        {
            targets: -1,
            orderable: false,
            width: '4%',
            ...
            //defaultContent: '<div class="d-flex order-actions">' +
            //    '<button class="btn btn-sm btn-outline-primary px-2 mx-2" id="btn_edit_cs_key"><i class="fa fa-pencil mr-2"></i>Edit</button>' +
            //    '<button class="btn btn-sm btn-outline-danger px-1 mx-2" id="btn_delete_cs_key"><i class="fa fa-trash mr-2"></i>Delete</button>' +
            //    '</div>'
            
        },
    ],
});
/* 08-11-2023 added by Binoy 
   To include space under search filter in the datatables
*/
